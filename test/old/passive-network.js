...
const { Network } = require( "../../lib/network/receiving/index" );


describe( "receiving network", () => {
	...
	it( "can be created", done => {
		...
		network.once( "listening", options => {
			if ( options.host === "0.0.0.0" ) {
				...
				// -> use loop-back IP 127.0.0.1 instead
	...
	it( "accepts client connections", () => {
		return PromiseUtil.map( clients, client => PromiseUtil.promisify( setupClient )( client ) );
	} );
	...
	it( "can send a message and reaches connected client", () => {
		return PromiseUtil.each( clients, client => PromiseUtil.promisify( ( client, cb ) => {
	...
	it( "can receive an invalid message", done => {
		...
		clients[0].conn.write( Buffer.from( [0xc1] ) );
	} );
	...
	it( "can finish", done => {
		network.end();
		network.once( "close", done );
		clients.forEach( client => client.conn.end() );
	} );

	/**
	 * Commonly sets up provided client.
	 *
	 * @param {object} client
