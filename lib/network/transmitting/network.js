...
class TransmittingNetwork extends Network {
	...
	constructor( address, options = {} ) {
		...
		Debug( `create request emitter for ${normalized.id} w/ options %j`, _options );
	...
	_write( message, _, doneFn ) {
		...
		if ( this.ended ) {
			Debug( `REQUEST to ${normalized.to.id} via ended network IGNORED` );
		} else {
			...
			Debug( `REQUEST to ${peerId}: %j`, normalized );
			...
			if ( !connection ) {
				...
				this.once( "close", () => { connection.end(); } );
			}

			connection.write( normalized, doneFn );
		}
