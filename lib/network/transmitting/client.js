...
class NetworkTcpClient extends Duplex {
	...
	/**
	 * Stops timer emitting event after client has been inactive for some time.
	 *
	 * @returns {void}
	 */
	...
	_write( message, _, callback ) {
		...
		try {
			normalized = NetworkMessage.normalize( message );
		} catch ( error ) {
			callback( error );
			return;
		}
		...
		if ( this._encodingTx ) {
			Debug( `SENDING to ${this.id}: %j`, normalized );

			// currently connected with peer -> transmit
			this._encodingTx.write( NetworkMessage.copyToSerializable( normalized ), error => {
				if ( error ) {
			...
			Debug( `FAILED SENDING to disconnected peer at ${this.id}: %j`, normalized );

			// locally simulate reply with error message
			process.nextTick( () => {
				this.push( Object.assign( NetworkMessage.deriveResponse( normalized, ["id"] ), {
					type: "reply",
					error: "not connected",
					fake: true,
					params: {
