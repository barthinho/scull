...
class NetworkTcpClient extends Duplex {
	...
	constructor( remoteAddress, options ) {
		...
		this._reconnect = Reconnect( reconnectAddress => {
			Debug( `connecting with ${reconnectAddress}` );

			return Net.connect( reconnectAddress.toSocketOptions() );
		} )( RECONNECT_OPTIONS, tcpSocket => {
			Debug( `connected with ${this.id}` );
			...
			/**
			 * Exposes limited view on underlying TCP socket currently in use
			 * for unit-testing purposes, only.
			 *
			 * @name NetworkTcpClient#_socket
			...
			.on( "connect", ( ...args ) => this.emit( "connect", ...args ) )
			.on( "reconnect", ( ...args ) => {
				Debug( `need to reconnect with ${this.id}` );
				this.emit( "reconnect", ...args );
			} )
			.on( "disconnect", () => {
				Debug( `disconnected from ${this.id}` );
				this._encodingTx = undefined;
				this.emit( "disconnect" );
			} );

		// start (re-)connecting with peer
		this._reconnect.connect( this.address );
	}
