	...
	_connect() {
		...
		this._reconnect = reconnect( reconnectOptions, ( peerRawConn ) => {
			...
			peerRawConn.on( 'close', () => {
				this._out = undefined;
				timers.clearTimeout( inactivityTimeout );
			} );
			...
			function resetInactivityTimeout() {
				...
				inactivityTimeout = timers.setTimeout( onInactivityTimeout, peer._options.inactivityTimeout );
			}

			function onInactivityTimeout() {
				peer.emit( 'inactivity timeout' );
			}
		} )
