...
module.exports = class PeerLeader extends EventEmitter {
	...
	_installSnapshot() {
		...
		SnapshotLog( "%s: _installSnapshot on %s", me, myPeer );
		...
		node.rpc( {
			...
			.then( reply => {
				SnapshotLog( "%s: got InstallSnapshot probe reply", me, reply );
			...
			.catch( error => {
				SnapshotLog( "%s: probing for InstallSnapshot failed", me, error );
		...
		/**
		 * Starts pushing chunks of snapshot after receiving positive reply to
		 * initial request for pushing snapshot.
		 *
		 * @returns {void}
		 */
		function installSnapshotAcknowledged() {
			...
			function installSnapshotPushCollectedChunk( data ) {
				...
				SnapshotLog( "%s: leader sending %s %d chunks %j", me,
				...
				if ( data.finished ) {
					...
					params.leaderId = me;
					params.peers = node.peers.addresses
				...
				node.rpc( {
					...
					.then( reply => {
						SnapshotLog( "%s: got InstallSnapshot reply", me, reply );

						if ( data.finished ) {
							// peer replied to last chunk of snapshot pushed
							// -> whole snapshot succeeded
							SnapshotLog( "%s: data finished, setting next index of %s to %d",
					...
					.catch( error => {
						SnapshotLog( "%s: InstallSnapshot failed", me, error );
