...
module.exports = class DB {
	/**
	 * @param {string} _location location of database, e.g. pathname of its containing folder
	 * @param {string|Address} id ID of node this database is used for
	 * @param {LevelDown} db database to use explicitly instead of local file-based LevelDOWN
	 * @param {object} options options for customizing used LevelDOWN instance
	 */
	constructor( _location, id, db, options ) {
		const levelDown = db || LevelDown;

		options = Object.assign( {}, options, { db: levelDown } );
		id = Address( id );
		...
		const dbName = id.toString().replace( /[/.]/g, "_" );
		...
		Object.defineProperties( this, {
			...
			levelDown: { value: levelUp.db },
		} );

		Object.defineProperties( this, {
			log: { value: Object.assign( this.db.sublevel( "log" ), {
				toJSON: () => "log",
			} ) },
			meta: { value: Object.assign( this.db.sublevel( "meta" ), {
				toJSON: () => "meta",
			} ) },
			state: { value: Object.assign( this.db.sublevel( "state" ), {
				toJSON: () => "state",
				clear: ClearDB,
			} ) },
		} );
				...
				stream.pipe( ConcatStream( entries => {
					resolve( entries
						.map( ( { key, value: c } ) => {
							...
							const t = parseInt( keyParts[0] ) || 0;
							const i = parseInt( keyParts[1] ) || 0;
