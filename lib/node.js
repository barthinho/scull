...
module.exports = class Node extends EventEmitter {
	...
	transition( state, onEnteringElection = false ) {
		Debug( '%s: switch to %s?', this.id, state );
		if ( onEnteringElection || state !== this._stateName ) {
			Debug( '%s: becomes %s%s', this.id, state, onEnteringElection ? ' due to election' : '' );
			...
			switch ( state ) {
				...
				case 'follower' :
					if ( this._electing ) {
						...
						if ( this._leaderId ) {
							Debug( '%s: mark election of new leader %s', this.id, this._leaderId );
							...
							this.once( 'new leader', newLeader => {
								if ( !this._electing ) {
									...
									Debug( '%s: deferredly mark election of new leader %s', this.id, this._leaderId );
	...
	isMajority( count, consensus = this.peers ) {
		Debug( '%s: got %d out of %d possible votes', this.id, count, consensus.length + 1 );
