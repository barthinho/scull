...
const ConsensusDebug = Debug( "scull:consensus" );
...
class Node extends EventEmitter {
	...
	updateOnRequest( from, action, params ) {
		if ( params ) {
			if ( params.term < this.term ) {
				ErrorLog( `${this.id}: rejecting message of previous term ${params.term}, requiring ${this.term} at least` );
				return Promise.reject( new Error( "message for outdated term rejected" ) );
			}
	...
	command( command ) {
		...
		function runNextCommand() {
			...
			console.log( `${that.id.port} START COMMAND attempt #${attempts} started %j`, _command );
			...
			if ( that._state instanceof States.Leader ) {
				...
				// -> wait for leader
				console.log( `${that.id.port} PAUSE COMMAND started %j`, _command );

				that.once( "new leader", newLeaderId => {
					console.log( `${that.id.port} RESUME COMMAND started %j`, _command );
			...
			function commandSucceeded( data ) {
				console.log( `${that.id.port} SUCCESS COMMAND started %j`, _command );
				onResolve( data );
			...
			function commandFailed( error, forceRejection = false ) {
				console.log( `${that.id.port} FAILURE COMMAND started %j: %s`, _command, error.message );
	...
	isMajority( proVotes, count, voterCount = null ) {
		...
		ConsensusDebug( `${this.id}: got ${count} out of ${_count} possible ${proVotes ? "pro-" : "contra-"}votes` );
