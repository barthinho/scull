...
class Node extends EventEmitter {
	...
	_setPeers( peers ) {
		...
		this._peers = peers.filter( p => p !== myAddress );
	 ...
	 * @private
	 */
	_transition( state, onEnteringElection = false ) {
	...
	_setTerm( term ) {
		if ( typeof term !== 'number' ) {
			throw new TypeError( 'term needs to be a number and was %j', term );
		}
			...
			switch ( command.type ) {
				case 'join' :
					...
					if ( this._peers.indexOf( command.peer ) < 0 && command.peer !== this.id.toString() ) {
						resultingPeersList = this._peers.concat( command.peer );
				...
				case 'leave' :
					...
					if ( this._peers.indexOf( command.peer ) > -1 && command.peer !== this.id.toString() ) {
						resultingPeersList = this._peers.filter( p => p !== command.peer );
						consensuses.push( resultingPeersList );
					}
					break;
			}
			...
			this._state.command( consensuses, command, options, ( err, result ) => {
				...
				if ( err ) {
					...
					if ( resultingPeersList ) {
						this._peers = resultingPeersList;
					}
