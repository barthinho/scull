...
module.exports = class NodeState extends EventEmitter {
	...
	_requestVoteReceived( message ) {
		...
		Debug( "%s: got request for voting: %j", node.id, message );
		...
		if ( requestedTerm >= currentTerm ) {
			if ( requestedTerm > currentTerm || !votedFor || ( votedFor === sender ) ) {
				if ( params.lastLogIndex >= node.log.stats.lastIndex ) {
					ConsensusDebug( `${me}: vote GRANTED for ${sender} to lead in term #${requestedTerm} while in term #${currentTerm}` );
					return true;
				}

				ConsensusDebug( `${me}: vote for ${sender} to lead in term #${requestedTerm} while in term #${currentTerm} REJECTED due to more advanced log` );
			} else {
				ConsensusDebug( `${me}: vote for ${sender} to lead in term #${requestedTerm} while in term #${currentTerm} REJECTED due to having voted for different leader before` ); // eslint-disable-line max-len
			}
		} else {
			ConsensusDebug( `${me}: vote for ${sender} to lead in term #${requestedTerm} while in term #${currentTerm} REJECTED due to obsolete term` );
		}
