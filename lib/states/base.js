...
class NodeState extends EventEmitter {
	...
	handleAppendEntries( from, params ) {
		...
		if ( prevLogIndex > -1 ) {
			...
			if ( validContinuation ) {
				...
				// es kommt vor, dass ein IS erfolgreich durchläuft, danach aber diese Prüfung hier immer wieder scheitert
		...
		function reply( issue, previousEntry = null ) {
			...
			if ( result.issue ) console.log( `${me.port} REPLY AppendEntries %j started`, result );
	...
	handleInstallSnapshot( from, params ) {
		...
		if ( params.done ) {
			...
			tasks.push( () => {
				console.log( `${node.id.port} receiving from ${from.port}: %j`, params );
