...
class NodeStateLeader extends NodeState {
	 ...
	 */
	seekConsensus( consensuses, command ) {
		const { log } = this.node;
		const index = log.push( command.toJSON() );

		ConsensusLog( `SEEKING for ${command} at log #${index}` );

		return PromiseUtil.each( consensuses, ( consensus, cIndex ) => this._waitForConsensus( index, command.options, consensus, cIndex + 1 ) )
			.then( () => {
				ConsensusLog( `REACHED for ${command} at log #${index}` );
