...
module.exports = class NodeStateLeader extends NodeState {
	...
	start() {
		const { node } = this;
		const peers = node.peers.addresses.map( address => String( address ) );
		const followers = {};

		for ( let i = 0, length = peers.length; i < length; i++ ) {
			const address = peers[i];
		...
		this._waitForConsensus( node.log.stats.committedIndex, {}, peers )
			.then( () => {
				ConsensusDebug( `REACHED on ${node.id} becoming leader w/ log index #${node.log.stats.committedIndex}` );
			} )
			.catch( error => {
				ConsensusDebug( `FAILED on ${node.id} becoming leader w/ log index #${node.log.stats.committedIndex}: ${error.stack}` );
			} );
	}
	 ...
	 *
	 * @note This method might require multiple confirmations on consensus using
	 *       different sets of peers asked for consensus (e.g. on commands for
	 *       joining or leaving cluster there are two requests: one w/ and one
	 *       w/o the joining/leaving node). That's controlled by list of lists
	 *       of peers w/ each list representing separate demand for consensus.
	 *
	 * @param {Array<Address>[]} consensuses one or more sets of peers required to confirm command prior to performing it
	 * @param {object} command actual command to be performed
	 * @param {WaitForConsensusOptions} options options customizing required consensuses
	 * @returns {Promise} promises command persistently tracked in log after reaching consensuses from cluster
	 */
	command( consensuses, command, options ) {
		const log = this.node.log;
		const index = log.push( command );

		ConsensusDebug( `SEEKING for command "${command.type}" at log index #${index}` );

		return PromiseUtil.each( consensuses, ( consensus, cIndex ) => this._waitForConsensus( index, options, consensus, cIndex + 1 ) )
			.then( () => {
				ConsensusDebug( `REACHED for command "${command.type}" at log index #${index}` );
			} )
			.then( () => log.commit( index ) )
			.then( () => undefined );
	}
	...
	_waitForConsensus( waitingForIndex, options, consensusPeers, consensusIndex = 1, numConsensuses = 1 ) {
		return new Promise( ( resolve, reject ) => {
			...
			ConsensusDebug( `SEEKING ${consensusIndex}/${numConsensuses} on log index #${waitingForIndex} from ${consensusPeers.length} remote voters` );
			...
			function onPeerCommit( peerController, logIndexCommittedByPeer ) {
				if ( logIndexCommittedByPeer >= waitingForIndex ) {
					...
					ConsensusDebug( `VALID VOTE for consensus on log index #${logIndexCommittedByPeer} from ${peerController.peerAddress}` );
					...
					ConsensusDebug( `OBSOLETE VOTE for consensus on log index #${logIndexCommittedByPeer} from ${peerController.peerAddress}` );
				}


				if ( node.isMajority( true, votes, voters.length ) && !demandVoteFrom.length ) {
					ConsensusDebug( `PARTIALLY REACHED ${consensusIndex}/${numConsensuses} on log index ${waitingForIndex}` );
					cleanup();
					resolve();
				}
			}
