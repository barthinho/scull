...
module.exports = class NodeStateLeader extends NodeState {
	 ...
	 *
	 * @param {Address[][]} consensuses list of lists of peers to be asked for consensus before processing command
	...
	command( consensuses, command, options ) {
		...
		return new Promise( ( resolve, reject ) => {
			Async.eachSeries( consensuses, this._waitForConsensus.bind( this, index, options ), error => {
				if ( error ) {
					reject( error );
				} else {
					log.commit( index ).then( resolve, reject );
				}
			} );
		} );
	}
