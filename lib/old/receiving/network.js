...
class ReceivingNetwork extends Network {
	...
	constructor( address = null, options = {} ) {
		...
		if ( options && options.server && ( !_address || typeof _address === "object" ) ) {
			_address = Object.assign( {}, _address || {}, options.server );
		}

		// normalize provided address
		_address = Address( _address );

		const localIp = _address.toSocketOptions().host;
		switch ( localIp ) {
			case "0.0.0.0" :
			case "::" :
		...
		Debug( `create request listener at ${_address.id} w/ options %j`, _options );
		...
		const serverAddress = _address.toSocketOptions();
		const listenerAddress = _options.server.public || { host: serverAddress.host };

		const listener = new NetworkTcpServer( this, Object.assign( {}, _options.server, serverAddress, listenerAddress ) )
			.on( "data", message => {
				const node = this.nodes[_address.id];
	...
	node( address, { createIfMissing = true } = {} ) {
		const _address = Address( address ).id;

		let node = this.nodes[_address];
		if ( !node && createIfMissing ) {
			Debug( `enabling reception of messages from ${_address} at ${this.server.id}` );

			node = new NetworkNode( _address, this, this.options )
				.once( "finish", () => { this.nodes[_address] = undefined; } );

			this.nodes[_address] = node;
		}
	...
	drop( address, options = {} ) { // eslint-disable-line no-unused-vars
		...
		if ( this.isValidNode( _address ) && !_address.matches( this.address ) ) {
			return new Promise( resolve => {
				const node = this.node( _address );
	...
	_write( message, _, doneFn ) {
		...
		try {
			_message = NetworkMessage.normalize( _message );
		} catch ( error ) {
			onWriteError( error );
			return;
		}

		if ( !this.isValidNode( _message.from ) ) {
			onWriteError( new Error( "REPLY from unknown node REJECTED" ) );
			return;
		}

		if ( !this.address.matches( _message.from ) ) {
			onWriteError( new Error( "REPLY on behalf of foreign node REJECTED" ) );
			return;
		}

		if ( !this.isValidNode( _message.to ) ) {
			onWriteError( new Error( "REPLY to unknown node REJECTED" ) );
			return;
		}

		Debug( `REPLY to ${_message.to}: %j`, _message );

		this.server.write( _message, doneFn );
